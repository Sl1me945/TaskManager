    ...
    public class AppRunner
    {
        ...
        private async Task AddTaskAsync()
        {
            ...
            try
            {
                ...
                if (task != null)
                {
                    task.AssignToUser(_currentUserId);
        ...
        private async Task UpdateTaskAsync()
        {
            ...
            try
            {
                ...
                {
                    taskToUpdate.SetPriority(priorityChoice switch
                    {
                        "1" => Priority.Low,
                        "2" => Priority.Medium,
                        "3" => Priority.High,
                ...
                if (taskToUpdate is WorkTask workTask)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(newProjectName))
                        workTask.UpdateProjectName(newProjectName);
                }
                else if (taskToUpdate is RecurringTask recurringTask)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(newIntervalInput) && int.TryParse(newIntervalInput, out var days))
                        recurringTask.UpdateRepeatInterval(TimeSpan.FromDays(days));
                }
