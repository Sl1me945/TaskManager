...
namespace ToDoApp.Infrastructure.Repositories
{
    public class FileUserRepository : IUserRepository
    {
        ...
        private static User MapToDomain(FileUserDto dto)
        {
            var user = new User(dto.Id, dto.Username, dto.PasswordHash );
            return user;
        }
        private static FileUserDto MapToDto(User user)
        {
            var dto = new FileUserDto
            {
                Id = user.Id,
        ...
        private async Task<List<User>> LoadAllUsersAsync()
        {
            try
            {
                var dtos = await _fileStorage.LoadAsync<List<FileUserDto>>(_filePath);
